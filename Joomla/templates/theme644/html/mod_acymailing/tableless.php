...
$document->addScript('templates/'.$template.'/js/cookie.js');
$path_url = JURI::current();

	$boxwidth = $params->get('boxwidth','250');
	$boxheight = $params->get('boxheight','250');
	$settimeout = ($params->get('settimeout','16') * 1000);
	$setinterval = ($params->get('setinterval','20') * 100000);
	$expires = $params->get('expires','7');
	//print_r($boxheight);
...
<?php if($params->get('effect','normal') == 'popup-effect'){ ?>
<script>
  jQuery(document).bind('ready', function(){
    var cc = jQuery.cookie('newsletter');
	if (cc == 'ok') {
		clearTimeout(timerId);
	} else  {
		
		var timerId = setTimeout(function tick() {
	  		jQuery( ".popbtn a" ).on( "click", function() {
			 	jQuery('.popupletter').addClass('zindex');
			});
			jQuery( ".popbtn a" ).trigger( "click" );
	 		timerId = setTimeout(tick, <?php echo $setinterval ?>);
		}, <?php echo $settimeout ?>);
		jQuery('#myModal .close').click(function() {
			jQuery('.popupletter').removeClass('zindex');
		});
	}
	jQuery('.btndontshow').click(function() {
		jQuery('.popupletter').removeClass('zindex');
		jQuery.cookie('newsletter', 'ok', { expires: <?php echo $expires ?> });
	});
...
<div class="popupletter">
	<div class="popbtn" style="display:none;">
		<a href="#myModal" role="button" class="btn" data-toggle="modal"><?php echo JText::_('TPL_BUTTON_NEWSLETTER') ?></a>
	</div>


	<div style="width:<?php echo $boxwidth.'px' ?>;" id="myModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	  <div style="width:<?php echo $boxwidth.'px' ?>;height:<?php echo $boxheight.'px' ?>;" class="modal-dialog" role="document">
	  <div class="modal-header">
	    <button class="close" aria-hidden="true" data-dismiss="modal" type="button">×</button>
	    <h3 id="myModalLabel"><?php echo JText::_('TPL_TITLE_NEWSLETTER') ?></h3>
	  </div>
<?php } ?>
...
<?php if($params->get('effect','normal') == 'popup-effect'){ ?>
<button type="button" class="btndontshow" data-dismiss="modal" aria-hidden="true"><?php echo JText::_('TPL_BUTTON_NEWSLETTER_DONT') ?></button>
</div>
</div>
</div>
<?php } ?>
